...
export default function buildSDF (glyph: Array<number>, offset: [number, number],
  scale: number, globalPos: number, extent: number): Path {
  ...
  const cursor: Cursor = { x0: 0, y0: 0, x: 0, y: 0, x1: 0, y1: 0, x2: 0, y2: 0, anchor: 0, indexPos: -1, globalPos }
  let i = 0
  ...
  while (i < len) {
    ...
    if (cmd === 0) { // moveTo
      ux0 = glyph[i++]
      uy0 = glyph[i++]
      cursor.x0 = ux0 / extent * scale + offset[0] + globalPos
      cursor.y0 = uy0 / extent * scale + offset[1] + globalPos
      ax = cursor.x0
      ay = cursor.y0
      ...
      ux0 = glyph[i++]
      uy0 = glyph[i++]
      cursor.x = ux0 / extent * scale + offset[0] + globalPos
      cursor.y = uy0 / extent * scale + offset[1] + globalPos
      _lineTo(cursor, res)
    } else if (cmd === 2) { // cubicBezierTo
      cursor.x1 = glyph[i++] / extent * scale + offset[0] + globalPos
      cursor.y1 = glyph[i++] / extent * scale + offset[1] + globalPos
      cursor.x2 = glyph[i++] / extent * scale + offset[0] + globalPos
      cursor.y2 = glyph[i++] / extent * scale + offset[1] + globalPos
      ux0 = glyph[i++]
      uy0 = glyph[i++]
      cursor.x = ux0 / extent * scale + offset[0] + globalPos
      cursor.y = uy0 / extent * scale + offset[1] + globalPos
      ...
      res.stroke.push(...fromLine([cursor.x0, cursor.y0], [ax, ay], globalPos))
      ...
      cursor.x1 = (ux0 + glyph[i++]) / extent * scale + offset[0] + globalPos
      cursor.y1 = (uy0 + glyph[i++]) / extent * scale + offset[1] + globalPos
      cursor.x2 = (ux0 + glyph[i++]) / extent * scale + offset[0] + globalPos
      cursor.y2 = (uy0 + glyph[i++]) / extent * scale + offset[1] + globalPos
      ux0 += glyph[i++]
      uy0 += glyph[i++]
      cursor.x = ux0 / extent * scale + offset[0] + globalPos
      cursor.y = uy0 / extent * scale + offset[1] + globalPos
      ...
      uy0 = glyph[i++]
      cursor.x = cursor.x0
      cursor.y = uy0 / extent * scale + offset[0] + globalPos
      _lineTo(cursor, res)
      ...
      cursor.x2 = glyph[i++] / extent * scale + offset[0] + globalPos
      cursor.y2 = glyph[i++] / extent * scale + offset[1] + globalPos
      cursor.x = glyph[i++] / extent * scale + offset[0] + globalPos
      cursor.y = glyph[i++] / extent * scale + offset[1] + globalPos
      _cubicTo(cursor, res)
    } else if (cmd === 14) { // partialCubicBezierTod delta
      // TODO: flip x1 and y1
      cursor.x2 = cursor.x0 + (glyph[i++] / extent * scale + offset[0] + globalPos)
      cursor.y2 = cursor.y0 + (glyph[i++] / extent * scale + offset[1] + globalPos)
      ux0 += glyph[i++]
      uy0 += glyph[i++]
      cursor.x = (ux0 + glyph[i++]) / extent * scale + offset[0] + globalPos
      cursor.y = (uy0 + glyph[i++]) / extent * scale + offset[1] + globalPos
      _cubicTo(cursor, res)
...
function _lineTo (cursor: Cursor, res: Path) {
  // grab constants
  const { x0, y0, x, y, globalPos, anchor } = cursor
  const { vertices, indices, stroke } = res
  // store vertices
  vertices.push(x, y, 0)
  // store stroke
  stroke.push(...fromLine([x0, y0], [x, y], globalPos))
...
function _quadraticTo (cursor: Cursor, res: Path) {
  // grab constants
  const { x0, y0, x1, y1, x, y, anchor, globalPos } = cursor
  ...
  for (let v = 1; v <= 5; v++) {
    ...
    stroke.push(...fromLine([xPrev, yPrev], [xCurr, yCurr], globalPos))
    xPrev = xCurr
    yPrev = yCurr
  }
  // update current to end
